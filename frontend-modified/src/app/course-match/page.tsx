...
const CourseMatchInputSchema = z.object({
...
export default function CourseMatchPage() {
  ...
  async function onSubmit(values: CourseMatchInput) {
    ...
    try {
      // Call real backend: POST /api/recommendations
      const result = await fetchRecommendations(values.description);
  ...
  const exampleDescription =
    "I'm a high school student with strong grades in Math and Physics. I love coding, building small robots, and I'm fascinated by artificial intelligence. I'm looking for an undergraduate program at a top-tier university that has a great reputation for engineering.";
    ...
    <div className="container mx-auto max-w-4xl py-12 px-4">
      <div className="text-center mb-12">
        ...
        <p className="mt-4 text-lg text-muted-foreground">
          Describe your interests and background. Our AI will suggest the best course matches for you.
        </p>
      </div>
            ...
            <CardFooter>
              <Button type="submit" disabled={isLoading} className="w-full md:w-auto">
                {isLoading ? (
                  ...
                  <><Zap className="mr-2 h-4 w-4" /> Find My Courses</>
                )}
              </Button>
            </CardFooter>
          </form>
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-sm font-semibold flex items-center gap-1">
                        <Star className="h-4 w-4 text-yellow-500" /> Match Score
                      </span>
                      <Badge variant={rec.matchScore > 80 ? 'default' : 'secondary'} className="bg-accent text-accent-foreground">
                        {rec.matchScore}%
                      </Badge>
                    </div>
